...
namespace TestTask.Minesweeper.Domain.Processors.Implementation
{
	...
	internal sealed class SnapshotProcessor : ISnapshotProcessor
	{
		...
		/// <inheritdoc/>
		public Snapshot Process(Snapshot previousSnapshot, Turn newTurn, out Enums.TurnResult newTurnResult)
		{
			...
			{
				newTurnResult = Enums.TurnResult.Defeat;
			...
			if (selectedCell.Value == Enums.CellValue.Empty)
			{
				...
				newTurnResult = Enums.TurnResult.EmptyCellOpened;
			}
			else
			{
				...
				newTurnResult = Enums.TurnResult.CellWithNumberOpened;
			}

			if (IsAllOpened(newSnapshot.Cells))
			{
				newTurnResult = Enums.TurnResult.Victory;
			}
